...
class interpolater():
    def __init__(self):
        ...
        self.interpolating_function=interpolate.splrep(self.frequency,self.responsivity,s=0)
        
    def convert(self,freq_list,volt_list):
    
        # This convert method assumes direct input from the recorded values.
        # Give UNCONVERTED frequencies (the same ones given to the hittite).
        ...
        real_freq=freq_list*12.0
        # Multiplies recorded frequency by 12 (since that is what the source does to the hittite freq).
        pow_multiplier=interpolate.splev(real_freq,self.interpolating_function,der=0)
        ...
        return pow_list, real_freq, pow_multiplier
        # Returns a tuple: power, adjusted frequency, pow_multiplier
